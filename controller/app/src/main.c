...
int main(void)
{
    ...
    while (1) {
        do {
            ...
            /** Calculate initial offset for potential controller misalignment. */
            if (offset_samples < OFFSET_SAMPLES_AMOUNT) {
                for (int i = 0; i < 3; i++) {
                    /** Cumulative average (Xn+1 + A * n) / (n + 1) */
                    attitude_offsets[i] =
                        ((attitude_from_gyro_deg[i] + attitude_offsets[i] * offset_samples)
                        / (offset_samples + 1));
                }
                offset_samples++;
                goto sleep;
            ...
            if (!offsets_logged) {
                LOG_INF("Attitude offsets: pitch=%.3f, roll=%.3f, yaw=%.3f",
                    attitude_offsets[0], attitude_offsets[1], attitude_offsets[2]);
                offsets_logged = true;
            }
            /** Apply offsets. */
            for (int i = 0; i < 3; i++) {
                attitude_from_gyro_deg[i] -= attitude_offsets[i];
            }

            if (i == 100) {
                LOG_INF("pitch=%.3f, roll=%.3f, yaw=%.3f",
                attitude_from_gyro_deg[0], attitude_from_gyro_deg[1], attitude_from_gyro_deg[2]);
            }
            /* Read inputs. */
            ...
            if (i == 100) {
                LOG_INF("PID pitch=%.3f, roll=%.3f, yaw=%.3f", pitch_pid, roll_pid, yaw_pid);
                LOG_INF("New input data: throttle=%.3f", input.throttle);
            }
            /** Get input. */
            ...
            if (i == 100) {
                LOG_INF("Throttle fl=%d, fr=%d, bl=%d, br=%d",
                    throttle_fl, throttle_fr, throttle_bl, throttle_br);
            }
sleep:
            k_sleep(K_MSEC(1));
